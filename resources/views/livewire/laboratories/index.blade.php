...
new #[Layout('layouts.app')] class extends Component {
    ...
    public ?int $laboratoryIdBeingDeleted = null;
    public string $laboratoryNameBeingDeleted = '';

    /**
     * Confirm the laboratory deletion.
     */
    public function confirmLaboratoryDeletion(int $id): void
    {
        $laboratory = Laboratory::findOrFail($id);
        $this->laboratoryIdBeingDeleted = $laboratory->id;
        $this->laboratoryNameBeingDeleted = $laboratory->name;
        $this->resetErrorBag();
        $this->dispatch('open-modal', 'confirm-laboratory-deletion');
    }

    /**
     * Delete/Archive a laboratory.
     */
    public function deleteLaboratory(LaboratoryService $laboratoryService): void
    {
        if (!$this->laboratoryIdBeingDeleted) {
            return;
        }

        $laboratory = Laboratory::findOrFail($this->laboratoryIdBeingDeleted);

        try {
            $laboratoryService->delete($laboratory);

            $this->dispatch('close-modal', 'confirm-laboratory-deletion');
            $this->reset(['laboratoryIdBeingDeleted', 'laboratoryNameBeingDeleted']);

            session()->flash('success', 'El laboratorio ha sido eliminado con éxito.');
    ...
    /**
     * Restore a soft-deleted/archived laboratory.
     */
    public function restoreLaboratory(int $id, LaboratoryService $laboratoryService): void
    {
        $laboratory = Laboratory::onlyTrashed()->findOrFail($id);
        $laboratoryService->restore($laboratory);

        session()->flash('success', 'El laboratorio ha sido restaurado con éxito.');
    }

    /**
     * Get the laboratories with pagination.
     */
...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-blue-900 dark:text-white tracking-tight">Gestión de Laboratorios</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-2">
                Administra el catálogo de laboratorios fabricantes de medicamentos para asegurar la trazabilidad y procedencia legal en el inventario.
            </p>
        </div>
        <div>
            <a href="{{ route('laboratories.create') }}" wire:navigate
    ...
    <div class="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 mb-6">
        <h2 class="text-lg font-bold text-blue-900 mb-4">Filtrar Laboratorios</h2>
    ...
    <div class="bg-white border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
        @if(Laboratory::withTrashed()->count() === 0)
            <div class="p-12 text-center">
                ...
                <h3 class="text-lg font-bold text-blue-900 mb-1">No hay laboratorios registrados</h3>
                <p class="text-sm text-slate-500 mb-6">Comienza registrando el primer laboratorio de medicamentos en el sistema.</p>
                <a href="{{ route('laboratories.create') }}" wire:navigate
        ...
        @elseif($laboratories->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-sm text-slate-500">No se encontraron laboratorios que coincidan con los filtros aplicados</p>
            </div>
        @else
                ...
                <table class="min-w-full divide-y divide-slate-100">
                    ...
                    <tbody class="bg-white divide-y divide-slate-100">
                        @foreach($laboratories as $laboratory)
                            <tr class="hover:bg-slate-50/50 transition-colors" wire:key="lab-{{ $laboratory->id }}">
                                ...
                                <td class="px-6 py-4 text-sm text-slate-600 max-w-xs truncate">
                                    {{ $laboratory->description ?: 'Sin descripción' }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                    @if($laboratory->trashed())
                                        <button type="button" 
                                            wire:click="restoreLaboratory({{ $laboratory->id }})"
                                    ...
                                    @else
                                        ...
                                        <button type="button" 
                                            wire:click="confirmLaboratoryDeletion({{ $laboratory->id }})"
    ...
    <x-modal name="confirm-laboratory-deletion" focusable>
        <div class="p-6">
            ...
            <p class="mt-2 text-sm text-slate-600 font-normal">
                Esta acción archivará el laboratorio <span class="font-semibold text-blue-950">"{{ $laboratoryNameBeingDeleted }}"</span>. Dejará de aparecer en los listados activos y formularios de registro, pero se mantendrá en el historial para fines de auditoría y consulta.
            </p>
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <x-danger-button wire:click="deleteLaboratory" class="cursor-pointer">
                    Eliminar
                </x-danger-button>
            </div>
        </div>
