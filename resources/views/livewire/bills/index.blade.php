...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-blue-900 dark:text-white">Facturas y Salidas de Mercancía</h1>
    ...
    <!-- Flash Message -->
    @if(session('success'))
        <div class="mb-6 p-4 bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 rounded-xl flex items-center gap-3 text-green-900 dark:text-green-200 text-sm font-medium">
    ...
    <!-- Filters Bar -->
    <div class="bg-white dark:bg-gray-800 border border-slate-100 dark:border-gray-700 shadow-sm rounded-2xl p-5 mb-6 space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            
            <!-- Search -->
            <div class="lg:col-span-2">
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Buscar Factura o Cliente</label>
                <div class="relative">
                    <input type="text" wire:model.live.debounce.300ms="search"
                        placeholder="N° Factura (FAC-000001), NIT o Cliente..."
                        class="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-gray-700/50 border border-slate-200 dark:border-gray-600 rounded-xl text-xs focus:ring-1 focus:ring-blue-900">
            ...
            <!-- Status Filter -->
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Estado</label>
                <select wire:model.live="status"
                    class="w-full py-2 px-3 bg-slate-50 dark:bg-gray-700/50 border border-slate-200 dark:border-gray-600 rounded-xl text-xs focus:ring-1 focus:ring-blue-900">
            ...
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Forma de Pago</label>
            ...
            <!-- Date From -->
            <div>
                <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Desde</label>
    ...
    <!-- Invoices Table -->
    <div class="bg-white dark:bg-gray-800 border border-slate-100 dark:border-gray-700 shadow-sm rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm border-collapse">
                <thead class="bg-slate-50 dark:bg-gray-700/50 border-b border-slate-200 dark:border-gray-700 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    <tr>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-gray-700">
                    @forelse($bills as $bill)
                        <tr class="hover:bg-slate-50/60 dark:hover:bg-gray-750 transition">
                            <!-- Invoice Number -->
                            <td class="py-3.5 px-4 font-mono font-bold text-xs text-blue-900 dark:text-blue-300">
                            ...
                            <!-- Total -->
                            <td class="py-3.5 px-4 text-right font-bold text-xs text-slate-900 dark:text-white">
        ...
        <!-- Pagination -->
        @if($bills->hasPages())
            <div class="px-5 py-3 border-t border-slate-100 dark:border-gray-700 bg-slate-50/50">
                {{ $bills->links() }}
            </div>
        @endif
    </div>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full shadow-2xl border border-slate-100 dark:border-gray-700 overflow-hidden"
                ...
                <div class="px-6 py-4 bg-slate-50 dark:bg-gray-750 border-b border-slate-100 dark:border-gray-700 flex items-center justify-between">
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="font-mono font-bold text-sm text-blue-900 dark:text-blue-300">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="p-3 bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 rounded-xl text-xs space-y-1">
                        <div class="font-bold text-slate-900 dark:text-white">{{ $this->viewingBill->customer?->name }}</div>
                    ...
                    @if($this->viewingBill->status === 'annulled')
                        <div class="p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl text-xs text-red-900 dark:text-red-200">
    ...
    @if($showAnnulModal && $this->annullingBill)
        <div class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full shadow-2xl border border-slate-100 dark:border-gray-700 p-6 space-y-4"
                ...
                <div class="text-center">
                    <h3 class="text-base font-bold text-slate-900 dark:text-white">¿Desea anular esta factura?</h3>
                ...
                <form wire:submit="annulBill" class="space-y-3">
                    <div>
                        ...
                        <textarea wire:model="annulmentReason" rows="3" required
                            ...
                            class="w-full text-xs p-2.5 border border-slate-200 rounded-xl focus:ring-1 focus:ring-red-500"></textarea>
